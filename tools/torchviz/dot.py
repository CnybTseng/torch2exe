...
def get_fn_name(fn, show_attrs, max_attr_chars):
    name = str(type(fn).__name__) + ':' + str(id(fn))
    if not show_attrs:
        return name
    attrs = dict()
    for attr in dir(fn):
...
def make_dot(var, ops, params=None, show_attrs=False, show_saved=False, max_attr_chars=50):
    ...
    def add_nodes(fn):
        ...
        fillcolor = 'yellow' if str(id(fn)) in ops else None
        dot.node(str(id(fn)), get_fn_name(fn, show_attrs, max_attr_chars), fillcolor=fillcolor)
